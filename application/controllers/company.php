...
class company extends CI_controller
{
	...
	public function display_company_info($list = '')
	{
		...
		foreach ($result as $item) {
			...
			$list .= $row_start;
			$list .= div(checkData($item['street_address_1']), $col_left);
			$list .= div(checkData($phone[0]), $col_right).$row_end.$row_start;
			$list .= div(checkData($item['street_address_2']), $col_left);
			$list .= div(checkData($phone[1]), $col_right).$row_end.$row_start;
			$list .= div(checkData($item['city']), $col_left);
			$list .= div('', $col_right);
	 ...
	 * --------------------------------------------
	 */
	public function display_company_opening_hrs($list = '')
	{
