...
class homebox extends CI_controller
{
	...
	public function homebox_title($title, $link)
	{
		...
		: $this->banner_title .= heading(anchor(
			$link, $title,array( 'title' => $title )), '2');
	...
	{
		($about == "") ?
		  $this->description .= heading(anchor('#',
		  	'no description available!'), '2')
		: $this->description .= div(character_limiter($about, 100),
			array( 'class' => 'text'));
	...
	{
		$span_class = array( 'class' => 'banner-icon '.$icon );
	...
	public function display_homebox_banner()
	{
		...
		foreach ($result as $item) {
			...
			$this->box .= common::create_homebox($item['type'], $contents);
			$this->clear_object();
			$contents  = '';
		}
